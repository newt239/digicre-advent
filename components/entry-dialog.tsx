...
export function EntryDialog({ selectedDay, onClose, existingEntry }: EntryDialogProps) {
  ...
  const handleDelete = async () => {
    ...
    const confirmed = window.confirm(`12月${selectedDay}日の記事を削除してもよろしいですか？`)
    if (!confirmed) return

    setIsDeleting(true)

    const result = await deleteEntry(selectedDay)
  ...
  return (
    <Dialog open={selectedDay !== null} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-2xl">12月{selectedDay}日の記事を登録</DialogTitle>
          <DialogDescription>投稿者名、記事タイトル、URLを入力してください</DialogDescription>
        </DialogHeader>

        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          <div className="space-y-2">
            <Label htmlFor="name">投稿者名 *</Label>
            <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="山田太郎" required />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Next.js 15の新機能について"
              required
            />
          </div>
